...
dograh_render_remote_nginx_conf() {
...

    {
        ...
        echo "    least_conn;"
        # `zone` (shared memory) is required for `resolve` below.
        echo "    zone dograh_api 64k;"
        # `resolve` (open-source nginx >= 1.27.3) makes each worker hostname
        # re-resolve through the `resolver` in the template, so a container
        # recreate that changes the api container's bridge IP does not leave
        # nginx proxying to a dead address ("502 no live upstreams").
